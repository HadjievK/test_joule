...
async def get_mcp_tools() -> list:
    # In testing mode, all tools (including HANA Cloud) are served from mcp-mock.json
    if os.environ.get("IBD_TESTING") == "1":
        mock_tools = _build_mock_tools()
        logger.info("Loaded %d mock tool(s) from mcp-mock.json (IBD_TESTING=1)", len(mock_tools))
        return mock_tools

    # Production: real SuccessFactors MCP tools + HANA Cloud direct-query tools
    from hana_tools import get_hana_tools
    hana_tools = get_hana_tools()
    logger.info("Loaded %d HANA Cloud tool(s)", len(hana_tools))
    ...
    try:
        ...
        return langchain_tools + hana_tools
    except Exception:
        logger.exception("Failed to load MCP tools from Agent Gateway — using HANA tools only")
        return hana_tools
