...
class TestA2AEndpoints:
    ...
    def test_agent_card_endpoint(self, start_agent):
        ...
        # Check status code
        assert status == 200, f"Agent card endpoint returned status {status}, response: {raw[:200]}"

        # Parse and validate JSON
        try:
            card_data = json.loads(raw)
        except ValueError as e:
            pytest.fail(f"Agent card endpoint returned invalid JSON: {e}. Response text: {raw[:200]}")
    ...
    #         pytest.fail(f"Could not connect to server on port {port}: {e}")
    #
    #     assert status == 200, f"Agent invocation returned status {status}. Response: {raw[:500]}"
    #
    #     try:
    #         result = json.loads(raw)
    #     except ValueError as e:
    #         pytest.fail(f"Agent invocation returned invalid JSON: {e}. Response text: {raw[:200]}")
    #
